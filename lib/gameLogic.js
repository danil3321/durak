 ...
 * или с одной из защитных карт (если они сыграны).
...
function processAttack(gameState, attackingCard, attackerId) {
  ...
  // Если стол не пуст, допустимые номиналы – это номиналы атакующих карт...
  const attackValues = gameState.table.map(pair => pair.attack.value);
  // ...а также номиналы защитных карт (если сыграны)
 ...
 * Проверяет, что все атаки защищены, добирает карты до 6 для каждого игрока,
...
function processBito(gameState) {
  ...
  // Добор карт для каждого игрока до 6
